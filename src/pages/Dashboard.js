...
const Dashboard = ({ selectedProduct, levelSimulation }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const summaeyData = await fetchDataApi(sessionStorage.getItem("name"), levelSimulation.name, selectedProduct.name)
      ...
      if (levelSimulation.name == 'Customer Level') {
        ...
        const simulationData = summaeyData.map(({ customerName,matcode, plant, lastYear, productSubCat, checked }) => {
          return {
            customerName: customerName,
            plant: plant,
            checked: checked,
          ...
          try {
            months.forEach((month, index) => {
              ...
              if(selectedProduct.name == 'Terry Towel' || selectedProduct.name == 'Bathrobe'){
                ...
                calculatedUVR = calculatedKGS !== 0.00 ? parseFloat(calculatedUSDN/calculatedKGS).toFixed(2): 0;               
              }
                ...
                calculatedUVR = calculatedSqMt !== 0.00 ? parseFloat(calculatedUSDN/calculatedSqMt).toFixed(2) : 0;  
              ...
              months.forEach((month, index) => {
                ...
                if (e.checked) {
                  ...
                  if (selectedProduct.name == 'Terry Towel' || selectedProduct.name == 'Bathrobe') {
                    ...
                    item.thisYear[month.toLowerCase() + 'Quantity'] = item.thisYear[month.toLowerCase() + 'Quantity'].toFixed(2);
                    handleColourChange(month, rowIndex, index, prevQuantity, simulatedQuantity, "SU/QTY");
                    calculatedKGS += parseFloat(item.thisYear[month.toLowerCase() + 'Quantity']);
                ...
                item.thisYear[month.toLowerCase() + 'UVR'] = (parseFloat(item.thisYear[month.toLowerCase() + 'quantity']) !== 0.00 ? parseFloat(item.thisYear[month.toLowerCase() + 'USDN']/item.thisYear[month.toLowerCase() + 'Quantity']).toFixed(2) : 0)
                // calculatedUVR = parseFloat(calculatedKGS).toFixed(2) !== 0.00 ? parseFloat(calculatedUSDN/calculatedKGS).toFixed(2) : 0
              })
  ...
  const handleSaveData = () => {
    try {
      ...
      setSavedData(dataTableAnnual.map(({ customerName, region, plant, productSubCat , lastYear, thisYear , wtPerPC}) => ({
        customerName: customerName,
        region: region,
        plant: plant,
        thisYear: thisYear,
      ...
      // console.log("this is the table to be saved",savedData)
  ...
  const headerGroup = (
    <ColumnGroup>
      <Row>
        ...
        <Column header={"Matcode"} field="matcode" scrollable frozen rowSpan={3}/>
        {years.map((year) => (
  ...
  return (
    <>
      <div className="grid">
        ...
        <h1 className="col-md-12 text-center text-color-secondary ml-4">Operational Level Simulation</h1>
